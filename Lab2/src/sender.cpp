...
void send_packet(Packet &pkt) {
  ...
  bool is_control_packet =
      (pkt.header.flags & (FLAG_SYN | FLAG_ACK | FLAG_FIN)) != 0;
  ...
  if (is_control_packet || !is_data_packet || !should_drop_packet()) {
    ...
    // cout << "[Sender] Simulated packet loss at seq=" << pkt.header.seq <<
    // endl;
  }
...
void timer_thread_func() {
  while (connection_active) {
    ...
    if (!oldest.acked &&
        chrono::duration_cast<chrono::milliseconds>(now - oldest.time_sent)
                .count() > TIMEOUT_MS) {
      ...
      // send_packet(oldest.pkt); // Only sending the oldest might not be enough
      // if pipe is empty, but standard Reno says Retransmit First Unacked.

      // CRITICAL FIX: To prevent domino timeouts for subsequent packets in the
      // window: When a timeout occurs, we must assume the network state has
      // changed or is unknown. We reset the timers for ALL outstanding packets
      // to prevent them from timing out instantly one after another as they
...
void listener_thread() {
  ...
  while (connection_active) {
    int bytes =
        recvfrom(sock, (char *)&sack_pkt, sizeof(SACKPacket), 0, NULL, NULL);
    if (bytes == SOCKET_ERROR)
      continue;

    size_t expected_size = sizeof(RDTHeader) + sack_pkt.header.len;
    if (calculate_checksum(&sack_pkt, expected_size) !=
        sack_pkt.header.checksum) {
      continue;
    }
    ...
    if (ack > base) {
      ...
      {
        // We use a temporary lock if strictly needed, but careful about lock
        // order. Assuming buffer_mutex is already locked by the caller (Wait,
        // listener_thread locks it at line 122) Yes, buffer_mutex is locked
        // from line 122 in original code. Note: The original code context for
        // this replacement is inside listener_thread logic. However,
        // listener_thread locks buffer_mutex at line 122. This block starts
        // around line 140. So we are inside the lock.
      ...
      if (in_fast_recovery) {
        if (ack >= recover) {
          ...
          if (verbose)
            cout << " -> (Full ACK, Exit Fast Recovery)" << endl;
          ...
          if (verbose)
            cout << " -> (Partial ACK, Stay in Fast Recovery)" << endl;

          // Improvements for Partial ACK:
          // 1. Deflate cwnd by amount of new data acknowledged? (Optional, kept
          // simple here to maintain throughput)
          // 2. IMMEDIATE RETRANSMIT of the next missing segment (which starts
          // at 'ack')
...
bool perform_handshake() {
  ...
  const int max_timeout_ms = TIMEOUT_MS * 16;

  while (attempt < max_attempts) {
    cout << "[Sender] Sending SYN (attempt " << (attempt + 1) << "/"
         << max_attempts << ", timeout " << current_timeout_ms << " ms)"
         << endl;

    send_packet(syn_pkt);

    auto deadline = chrono::steady_clock::now() +
                    chrono::milliseconds(current_timeout_ms);

    while (chrono::steady_clock::now() < deadline) {
      auto now = chrono::steady_clock::now();
      auto remaining =
          chrono::duration_cast<chrono::milliseconds>(deadline - now);
      ...
      if (activity == SOCKET_ERROR) {
        ...
        if (err == WSAEINTR)
          continue;
        cerr << "[Sender] select() failed during handshake. Error: " << err
             << endl;
        break;
      }
      ...
      if (activity > 0 && FD_ISSET(sock, &read_fds)) {
        ...
        int responder_len = sizeof(responder_addr);
        int res = recvfrom(sock, (char *)&candidate, sizeof(candidate), 0,
                           (sockaddr *)&responder_addr, &responder_len);
        if (res <= 0)
          continue;

        size_t expected_size = sizeof(RDTHeader) + candidate.header.len;
        if (calculate_checksum(&candidate, expected_size) !=
            candidate.header.checksum) {
          continue;
        }

        bool is_syn_ack =
            (candidate.header.flags & (FLAG_SYN | FLAG_ACK)) ==
            (FLAG_SYN | FLAG_ACK);
        if (is_syn_ack && candidate.header.ack == syn_pkt.header.seq + 1) {
          Packet syn_ack = candidate;
          server_addr = responder_addr;
          ...
          cout << "[Sender] Handshake complete after " << (attempt + 1)
               << " attempt(s)." << endl;
        ...
        // Unexpected packet during handshake; ignore but continue waiting.
      }
    }
    ...
    cout << "[Sender] Handshake timeout. Preparing retransmission..." << endl;
    current_timeout_ms = min(current_timeout_ms * 2, max_timeout_ms);
  }

  cout << "[Sender] Handshake failed after " << max_attempts
       << " attempts." << endl;
  return false;
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    ...
    if (win > 0 && win <= 256) {
      ...
      cerr << "正确格式: sender.exe [窗口] [丢包率] [文件] [本地IP] [本地端口] "
              "[目标IP] [目标端口]"
           << endl;
      cerr << "示例: sender.exe 32 0.0 test.jpg 127.0.0.1 12000 127.0.0.1 12001"
           << endl;
  ...
  if (argc > 6)
    target_ip = argv[6];
  if (argc > 7)
    target_port = atoi(argv[7]);
  if (argc > 8)
    verbose = (strcmp(argv[8], "-v") == 0);
  ...
  cwnd = min<uint32_t>(static_cast<uint32_t>(SEND_WINDOW_SIZE) * MSS,
                       static_cast<uint32_t>(MSS * 10));
  ...
  if (!infile.is_open()) {
    ...
    for (int i = 0; i < 1024; i++) {
      ...
      snprintf(line, sizeof(line),
               "Line %04d: This is test data for RDT protocol performance "
               "evaluation.\n",
               i);
  ...
  while (chrono::duration_cast<chrono::seconds>(chrono::steady_clock::now() -
                                                fin_start)
             .count() < 3) {
    fd_set read_fds;
    FD_ZERO(&read_fds);
    ...
    if (activity > 0) {
      int res =
          recvfrom(sock, (char *)&ack_of_fin, sizeof(Packet), 0, NULL, NULL);
  ...
  if (!received_ack) {
    cout << "[Sender] Warning: Did not receive ACK of FIN, continuing anyway..."
         << endl;
  }
  ...
  while (chrono::duration_cast<chrono::seconds>(chrono::steady_clock::now() -
    ...
    if (activity > 0) {
      int res =
          recvfrom(sock, (char *)&recv_fin, sizeof(Packet), 0, NULL, NULL);
